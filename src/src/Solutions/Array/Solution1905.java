...
public class Solution1905 {
    ...
    private boolean inBound(int x, int y, int totalRows, int totalCols){
        return 0 <= x && x < totalRows && 0 <= y && y < totalCols;
    }
    ...
    private boolean isSubIsland(int x, int y, int[][] grid1, int[][] grid2, boolean[][] visited){
        ...
        for (int[] direction: directions){
            ...
            if (inBound(nextX, nextY, totalRows, totalCols)
                    && !visited[nextX][nextY]
                    && isCellLand(nextX, nextY, grid2)){
                visited[nextX][nextY] = true;
                boolean nextCellIsPartOfSubIsland = isSubIsland(nextX, nextY, grid1, grid2, visited);
